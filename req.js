...
class api {
#key;
#base =   "http://noded1.iblgroup.cloud:30122/api";   
#base2 = "https://api.bronxyshost.com.br/api-bronxys";
...
vrcpf(cpf) {
return `${this.#base}/vr_cpf?cpf=${cpf}`;
}
...
async letra_m(letra) {
try {
var abc = await this.fetchJson(`${this.#base2}/letra_musica?letra=${letra}`);
return abc;
} catch (error) {
...
async gpt(q, token) {
try {
var abc = await this.fetchJson(`${this.#base2}/PERGUNTE_E_EU_RESPONDO?q=${q}&TOKEN_GPT=${token}`);
return abc;
} catch (error) {
